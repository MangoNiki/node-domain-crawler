(function() {
    ...
    function getPureCharacterDomains(nums, prefix, subfix) {
        var arr = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j',
                'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't',
                'u', 'v', 'w', 'x', 'y', 'z'
            ],
            domains = [];
        getPureCharacter('', nums, arr, prefix, subfix, domains);
        return domains;
    }

    function getPureCharacter(domain, count, chars, prefix, subfix, domains) {
        for (var i = 0, lenI = chars.length; i < lenI; i++) {
            if (count == 1) {
                domains.push(prefix + domain + chars[i] + subfix);
                continue;
            }
            getPureCharacter(domain + chars[i], count - 1, chars, prefix, subfix, domains);
        }
    }

    exports.start = function() {

        // var domains = getPureNumberDomain(0, 100, '', 'web');
        var domains = getPureCharacterDomains(2, '', 'blog');
